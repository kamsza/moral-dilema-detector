...
public class CollisionConsequencePredictor {
    ...
    public void createCollisionConsequences(Decision decision, Actor victimActor, Actor other) {
        ...
        for (Living_entity victim : individualVictims) {
            ConsequenceType consequenceType = getHealthConsequenceType(victimActor, other);
    ...
    private ConsequenceType getHealthConsequenceType (Actor victimActor, Actor other){
        double speedOfCollision = PhysicsUtils
                .GetRelativeSpeed(other.getRigidBody().getSpeed(), victimActor.getRigidBody().getSpeed())
                .getMagnitude();
